from sqlalchemy import create_engine, Column, String, DateTime, ForeignKey, Integer, UniqueConstraint, Float, Boolean
...
class Pedido(Base):
    ...
    # Relaciones
    estado_tramitacion_id = Column(Integer, nullable=False) #Estado actual
    estado_tramitacion = relationship("Estado")
...
class HistorialPedido(Base):
    ...
    id_transicion = Column(Integer, ForeignKey("transiciones.id"), nullable=False)
...
class Contrato(Base):
    ...
    cesta_general_id = Column(Integer, ForeignKey("cestas_generales.id_cesta_general"), nullable=True)
    cesta_general = relationship("CestaGeneral")

    moneda_id = Column(Integer, ForeignKey("monedas.id_moneda"), nullable=True)
    moneda = relationship("Moneda")

# Tabla de Cestas Generales

class CestaGeneral(Base):
    __tablename__ = "cestas_generales"

    id_cesta_general = Column(Integer, primary_key=True, autoincrement=True)
    nombre = Column(String, nullable=False)
    descripcion = Column(String, nullable=True)
    fecha_creacion = Column(DateTime, default=datetime.datetime.utcnow)
...
class Cesta(Base):
    ...
    usuario_sap_id = Column(Integer, ForeignKey("usuarios.id"), nullable=True)
